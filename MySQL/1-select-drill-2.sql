...
;
    ...
    Réponse 1 =  SELECT * FROM octocats;

    -- Affiche uniquement les prénoms
    Réponse 2 =  SELECT firstname FROM octocats;

    -- Affiche les prénoms, noms et age de chaque octocat
    Réponse 3 =  SELECT firstname, lastname, age  FROM octocats;

    -- Affiche les octocats dont le nom commence par 'N'
    Réponse 4 =  SELECT lastname LIKE 'N%'  FROM octocats;

    -- Affiche le prénom et le nom des octocats de la promo "promo1-central"
    Réponse 5 =  SELECT firstname, lastname  FROM octocats  WHERE promo='promo1-central';

    -- Affiche le prénom, le nom et l'année de naissance des octocats de la promo "promo1-anderlecht"
    Réponse 6 =  SELECT firstname, lastname, birthdate  FROM octocats  WHERE promo='promo1-anderlecht';

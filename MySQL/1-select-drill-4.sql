...
--
-- concat
-- Elle permet de concaténer des colonnes pour recevoir un tableau plus facile à traiter ensuite en PHP. Par exemple, tu peux faire ceci:
--
...
-- Pour en savoir plus sur ces fonctions, va voir sur openclassroom.

-- Drill 4
--
    -- Combien y a-t-il de garçons à Becode?
    Réponse 1 = SELECT COUNT(*) FROM octocats WHERE gender='M';

    -- Combien y a-t-il de filles à Becode/Central ?
    Réponse 2 = SELECT promo='promo1-central' ,COUNT(*) FROM octocats  WHERE gender= 'F'; -----> 8

    -- Combien y a-t-il de garçons à Becode/Central ?
    Réponse 3 = SELECT promo='promo1-central' ,COUNT(*) FROM octocats  WHERE gender= 'M'; ------> 24

    -- Combien y a-t-il d'octocats dont le prénom est 'Nadia' à becode?
    Réponse 4 = SELECT COUNT(*) FROM octocats WHERE firstname='Nadia';
...
-- exercice suivant '5' :
    ...
    -- Sur ce nouveau SQLFiddle, trouve la fonction permettant de n'afficher que l'année de la colonne "birthdate" et affiche le prénom de tous les octocts et leur année de naissance. (indice: comment dit-on "année" en anglais?)
    'Réponse 5 = SELECT firstname  FROM octocats ;-------> CONCAT ? ???'


-- Sur le sqlfiddle bac à sable de Météo:

    -- Trouve la fonction permettant de retourner un tableau contenant uniquement la température maximale prévue ("Demain le maxima observé en Belgique sera de ... degrés")
    Réponse 6 = SELECT MAX(haut)  FROM Météo ;  -------> 28

    -- Trouve la fonction permettant de retourner un tableau contenant uniquement la température minimale prévue ("Demain le minima observé en Belgique sera de ... degrés")
    Réponse 7 = SELECT MIN(bas)  FROM Météo ;  -------> 12

-- Aller plus loin par toi-même, C'est la fin du parcours SQL sur les SELECT. Tu en sais largement assez pour développer. Lorsque tu auras envie d'aller plus loin, voici des ressources très bien faites.

    Utiliser les fonctions SQL, sur openclassroom.
    Les bases du Select, expliqué par sqlcourse.com: Basics of Select
    Trucs et astuces avec Select, expliqué par sqlcourse.com: Powerful things to do with Select
